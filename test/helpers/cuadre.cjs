...
// Copia adaptada a amigable-123 (2026-09-24): amigable no manda `mes` en ventas ni
// liquidaciones; el mes sale de la fecha local de la venta.
...
async function cuadre(w, { stockEsperado } = {}) {
  ...
  const mesDe = (iso) => fechaLocal(iso).slice(0, 7);
  const ventas = (await w.request('/api/ventas/todas')).map((v) => ({ ...v, mes: v.mes || mesDe(v.fecha) }));
  ...
  // amigable: el precio INCLUYE el IVA (Ecuador); lo cobrado es ingresosConIva.
  const plCobrado = pl.ingresosConIva != null ? pl.ingresosConIva : pl.ingresos;
  if (ingresoHoy !== c2(plCobrado)) mal('R2 P&L cobrado', `ventas ${ingresoHoy / 100} vs P&L ${plCobrado}`);
  ...
  for (const l0 of liq) {
    const l = { ...l0, mes: l0.mes || mesActual };
    ...
    const delMes = ventas.filter((v) => v.ubicacionId === l.ubicacionId && v.mes === l.mes && v.modoComision !== 'counter' && v.netoCasa !== null && v.netoCasa !== undefined); // amigable: sin reparto = de la casa
    const aj = (l.ajustes || []);
