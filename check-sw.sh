...
sw_ver=$(grep -oE 'c123-shell-v[0-9]+' docs/sw.js | head -1)
vj_ver=$(grep -oE 'c123-shell-v[0-9]+' docs/version.json | head -1)
...
if [ -f docs/version-manifest.json ]; then
  desfasados=$(node -e '
    ...
    for(const rel of Object.keys(files)){
      ...
      // Pages sirve LF desde Git aunque el checkout Windows use CRLF (port de friendly-123).
