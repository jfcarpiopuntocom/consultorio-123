// worker.js — license ping for Consultorio 123.
...
function cors(resp) {
  ...
  // FIX (homologado de Consultorio 123, JFC 2026-07-28/29): DELETE was missing
...
/* ─────────────────────────────────────────────────────────────────────
   VERSION CONTROL FOR LICENSES (homologado de Consultorio 123, JFC 2026-07-28)

   Two real incidents in Consultorio 123: JFC's own name went blank once, and a
...
async function handleCheckin(req, env) {
  ...
  // Determine product
  const producto = "consultorio-123";

  const registro = {
    instanceId,
    producto,
    // FIX (homologado de Consultorio 123, JFC 2026-07-28): used "!= null" —
...
async function handleRecoverPin(req, env) {
  ...
  try {
    resendResp = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: `Consultorio 123 <${fromEmail}>`,
        to: [emailDestino],
        subject: "Tu clave de acceso — Consultorio 123",
        text: [
          `Tu clave de dueño en Consultorio 123 es: ${pinDisplay}`,
          "",
        ...
        html: [
          ...
          `Tu clave de dueño en <strong>Consultorio 123</strong> es:</p>`,
          ...
          `<p style="font-family:sans-serif;font-size:12px;color:#999;">— Consultorio 123</p>`,
        ].join(""),
      }),
    });
  } catch (err) {
...
// POST /editar-correo — master-only (homologado de Consultorio 123). Edits
...
export default {
  async fetch(req, env) {
    ...
    if (url.pathname === "/licencias" && req.method === "GET") {
      ...
      // FIX (homologado de Consultorio 123, JFC 2026-07-29: "se queda en vacio
