// borradores.js — consultorio-123 · Nada tecleado se pierde, nunca
// ============================================================================
...
(function () {
  ...
  function enganchar(cont, nombre, opciones) {
    ...
    try {
      ...
      cuando = mismoDia
        ? "de las " + d.toLocaleTimeString("es", { hour: "2-digit", minute: "2-digit" })
        : "del " + d.toLocaleDateString("es", { day: "numeric", month: "long" });
    } catch (_) {}
    msg.innerHTML =
      '<span style="color:var(--rust,#E86040);">Tenías datos sin guardar ' + cuando + ".</span> " +
      '<button type="button" data-borrador-si style="font-size:13px;padding:4px 10px;">Recuperarlos</button> ' +
      '<button type="button" data-borrador-no style="font-size:13px;padding:4px 10px;background:transparent;">Empezar de cero</button>';
...
(function () {
  ...
  /* Semilla propia de un consultorio. La de las apps hermanas es de tienda
     (Bar, Artesania, Juguetes) y aqui no significa nada: consultorio-123 no
     maneja perchas, maneja atenciones, insumos y servicios. */
  var SEMILLA = [
    "Consulta", "Procedimiento", "Control", "Terapia",
    "Examen de laboratorio", "Imagenología",
    "Medicamento", "Insumo médico", "Material descartable",
    "Ortesis y prótesis", "Vacuna",
    "Certificado médico", "Servicio",
  ];
  ...
  function enganchar(inp) {
    ...
    if (!inp.placeholder || /licores/i.test(inp.placeholder)) {
      inp.placeholder = "Elige una o escribe la tuya";
    }
  }
