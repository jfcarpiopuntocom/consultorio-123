/*!
 * plan-pagos.js — consultorio-123 · Cuotas y abonos irregulares
 * ============================================================================
 ...
 *     nadie contrajo. La deuda se contrajo entera el dia del cxc_cargo.
 ...
 *
 * ---------------------------------------------------------------------------
 * SIN PENALIDAD. Regla dura de JFC, igual que en nucleo-cxc.js: este modulo no
 ...
 * a favor, que es lo que nucleo-cxc.js ya sabe representar (saldo positivo).
 ...
 * lineas de sumarPeriodos(), y rrule pesa mas que todo nucleo-cxc.js junto. Estas
...
(function (global) {
  ...
  function hechosDe(pacienteId) {
    ...
    return global.AMG.Hechos.todos().then(function (todos) {
      return _sinDuplicados((todos || []).filter(function (h) {
        return String(_d(h).pacienteId || "") === String(pacienteId);
      }), "pacienteId");
    }).catch(function (e) {
  ...
  function planActivo(pacienteId) {
  ...
  function crearPlan(pacienteId, datos) {
    if (!pacienteId) return Promise.reject(new Error("plan-pagos: falta pacienteId"));
  ...
  function anularPlan(pacienteId, motivo) {
    if (!pacienteId) return Promise.reject(new Error("plan-pagos: falta pacienteId"));
    var payload = {
      pacienteId: String(pacienteId),
