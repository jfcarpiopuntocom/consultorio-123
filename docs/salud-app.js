/*!
 * salud-app.js — consultorio-123 · Autorreporte de fallas de la app
 * ============================================================================
        ...
        document.getElementById("oc-version-recargar").addEventListener("click", function () {
          ...
          try {
            caches.keys().then(function (ns) {
              return Promise.all(ns.filter(function (n) { return n.indexOf("c123-shell-") === 0; })
...
(function verificarIntegridadCacheada() {
  ...
  try {
    ...
    async function verificarUno(nombre) {
      try {
        var cache = await caches.open(/* misma CACHE activa */ (await caches.keys()).filter(function (n) { return n.indexOf("c123-shell-") === 0; }).pop() || "");
