/* PORTADO A consultorio-123 (Fase 4, JFC 2026-09-08). Claves c123_*.
   Capability B (verificacion de consistencia + auto-resync) funciona YA
   (usa huella/deviceIdActual/resincronizar/pedirCatalogo, que OCSync expone).
   Capabilities A y C (snapshot local / entre pares) quedan FAIL-SAFE no-op
   hasta que la Fase 3 agregue OCSync.estadoParaCheckpoint/aplicarCheckpoint/
   enviarMensaje. No rompe nada: cada capability guarda con if(!...) return. */
 ...
 *      (c123_micelio_vistos) con la propia; si un par diverge de forma
...
(function () {
  "use strict";
  var DB_NAME = "c123_sync_watchdog";
  var DB_VER = 1;
  ...
  function huellasDePares() {
    try {
      var raw = localStorage.getItem("c123_micelio_vistos") || "{}";
  ...
  /* FASE 2 (2026-08-27): la cola offline (c123_sync_cola) marcó un desborde
  ...
  function colaDesbordada() {
    try {
      var n = Number(localStorage.getItem("c123_sync_cola_desbordada") || 0);
      return n > 0 ? n : 0;
    } catch (_) { return 0; }
  }
