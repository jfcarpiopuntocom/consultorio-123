...
//   directamente.
// ===========================================================================
// MIGRACION UNA-VEZ (JFC, 2026-08-20): hasta hoy la activacion guardaba la
// licencia bajo "f123_owned" (clave de friendly-123) por un copy-paste no
// corregido. Los otros 6 archivos de esta app siempre leyeron "c123_owned",
// que nunca se escribia. Dispositivos activados ANTES de este fix tienen su
// licencia real solo en "f123_owned" -- sin esta migracion quedarian sin
// licencia (rescate, WhatsApp guardado, respaldo) al primer login tras
// actualizar. Se corre una sola vez: si ya existe "c123_owned" no hace nada.
try {
  if (!localStorage.getItem("c123_owned") && localStorage.getItem("f123_owned")) {
    localStorage.setItem("c123_owned", localStorage.getItem("f123_owned"));
  }
} catch (_) {}
...
(function () {
  ...
  function generarCodigoSync() {
    ...
    return "C123-" + completo.slice(0, 4) + "-" + completo.slice(4, 8) +
  ...
  function mostrarAvisoCaidaDemo() {
    try {
      ...
      var _esES = true; try { _esES = !window.OCI18n || window.OCI18n.getLang() !== "en"; } catch (_) {}
      t.innerHTML = '<span style="flex:1;">'
        + (_esES ? 'Ese PIN no se reconoció — estás viendo el modo demo. ' : "That PIN wasn't recognized — you're seeing demo mode. ")
        + '<button id="oc-aviso-demo-reintentar" style="background:none;border:none;color:#8ecbff;text-decoration:underline;font-weight:700;cursor:pointer;padding:0;font-size:14px;">' + (_esES ? 'Reintentar mi PIN' : 'Retry my PIN') + '</button></span>'
  ...
  function construirModalActivacion() {
    ...
    wrap.querySelector("#oc-act-confirmar").addEventListener("click", async function () {
      ...
      try { localStorage.setItem("c123_owned", JSON.stringify({ instanceId: idInstancia, email: email, whatsapp: telDigitos, activatedAt: Date.now(), syncCode: syncCode, licenseCode: licenseCode })); } catch (_) {}
      ...
      wrap.querySelector("#oc-act-exito-txt").innerHTML = _esES
        ? ("Tu PIN de dueño es <strong>" + ACTIVATION_PIN + "</strong> — puedes cambiarlo cuando quieras en Avanzado &rarr; Claves. " +
           ...
           "<br><br>Tu c&oacute;digo de licencia: <strong style='font-family:monospace;letter-spacing:.1em;font-size:18px;color:#E86040;'>" + String(syncCode).replace(/[&<>]/g, "") + "</strong>" +
           ...
           "<strong>Este c&oacute;digo es privado de tu equipo.</strong> Quien lo tiene, entra a tus datos. " +
           "Ap&uacute;ntalo en un lugar seguro y comp&aacute;rtelo solo dentro de tu equipo, uno a uno. As&iacute; no tienes que cambiarlo despu&eacute;s.</p>")
  ...
  function _ocMascaraCodigo(inp) {
    ...
    inp.setAttribute("maxlength", "25");        // C123- + 4 + 1 + 4 = 14
